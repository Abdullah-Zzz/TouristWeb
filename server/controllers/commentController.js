...
const addComment = async (req, res) =>{
    try{
        ...
        if(chkTripExits){
            ...
            if(chkComments){
                await schema.findByIdAndUpdate(chkComments._id, {$push: {comments:[{comment:comment,createdBy:userDetails.name, createdAt:String(Date.now()),rating:rating}]}}, {new:true})
                res.status(200).json("Comment Added")
            }
            else{
                const newComment = new schema({id,comments:[{comment:comment,createdBy:userDetails.name, createdAt:String(Date.now()),rating:rating}]})
